...
pub struct EngineManager {
  codex: Arc<Mutex<CodexEngine>>,
  claude: Arc<Mutex<ClaudeSidecarEngine>>,
}

impl EngineManager {
  pub fn new() -> Self {
    Self {
      codex: Arc::new(Mutex::new(CodexEngine::default())),
      claude: Arc::new(Mutex::new(ClaudeSidecarEngine::default())),
    }
  }

  pub async fn list_engines(&self) -> anyhow::Result<Vec<EngineInfoDto>> {
    let codex = self.codex.lock().await;
    let claude = self.claude.lock().await;

    Ok(vec![
      EngineInfoDto {
        id: codex.id().to_string(),
        name: codex.name().to_string(),
        models: codex.models().into_iter().map(|m| m.id).collect(),
      },
      EngineInfoDto {
        id: claude.id().to_string(),
        name: claude.name().to_string(),
        models: claude.models().into_iter().map(|m| m.id).collect(),
      },
    ])
  }
  ...
  pub async fn health(&self, engine_id: &str) -> anyhow::Result<EngineHealthDto> {
    match engine_id {
      "codex" => {
        let codex = self.codex.lock().await;
        let available = codex.is_available().await;
        let version = codex.version().await;
        Ok(EngineHealthDto {
          id: "codex".to_string(),
          available,
          version,
      ...
      "claude" => {
        let claude = self.claude.lock().await;
        let available = claude.is_available().await;
        let version = claude.version().await;
        Ok(EngineHealthDto {
          id: "claude".to_string(),
          available,
          version,
          details: Some("Claude sidecar scaffold is present; SDK runtime wiring pending".to_string()),
        })
      }
  ...
  pub async fn respond_to_approval(
    ...
    match thread.engine_id.as_str() {
      "codex" => {
        let codex = self.codex.lock().await;
        codex.respond_to_approval(approval_id, response).await
      }
      "claude" => {
        let claude = self.claude.lock().await;
        claude.respond_to_approval(approval_id, response).await
      }
      _ => anyhow::bail!("unsupported engine_id {}", thread.engine_id),
    }
  }
  ...
  pub async fn interrupt(&self, thread: &ThreadDto) -> anyhow::Result<()> {
    ...
    match thread.engine_id.as_str() {
      "codex" => {
        let codex = self.codex.lock().await;
        codex.interrupt(engine_thread_id).await
      }
      "claude" => {
        let claude = self.claude.lock().await;
        claude.interrupt(engine_thread_id).await
      }
      _ => anyhow::bail!("unsupported engine_id {}", thread.engine_id),
    }
  }
