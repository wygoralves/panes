...
pub async fn stage_files(
  ...
  tokio::task::spawn_blocking(move || repo::stage_files(&repo_path, &files).map_err(err_to_string))
    .await
    .map_err(|error| error.to_string())?
}
...
pub async fn unstage_files(
  ...
  tokio::task::spawn_blocking(move || repo::unstage_files(&repo_path, &files).map_err(err_to_string))
    .await
    .map_err(|error| error.to_string())?
}
