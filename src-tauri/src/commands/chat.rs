...
use crate::{
  db,
  engines::{ActionType, EngineEvent, OutputStream, SandboxPolicy, ThreadScope},
...
pub async fn send_message(
  ...
  let workspace = db::workspaces::ensure_default_workspace(&state.db).map_err(err_to_string)?;

  let mut thread = match db::threads::get_thread(&state.db, &thread_id).map_err(err_to_string)? {
    Some(thread) => thread,
    None if thread_id == "demo-thread" => {
      db::threads::ensure_demo_thread(&state.db, &workspace.id).map_err(err_to_string)?
    }
    None => return Err(format!("thread not found: {thread_id}")),
  };
  ...
  if thread.engine_thread_id.as_deref() != Some(&engine_thread_id) {
    db::threads::set_engine_thread_id(&state.db, &thread.id, &engine_thread_id).map_err(err_to_string)?;
...
pub async fn cancel_turn(state: State<'_, AppState>, thread_id: String) -> Result<(), String> {
  ...
  if let Some(thread) = db::threads::get_thread(&state.db, &thread_id).map_err(err_to_string)? {
    state.engines.interrupt(&thread).await.map_err(err_to_string)?;
  }
  ...
  while let Some(event) = event_rx.recv().await {
    ...
    if state.config.debug.persist_engine_event_logs {
      if let Ok(value) = serde_json::to_value(&event) {
        let _ = db::actions::append_event_log(&state.db, &thread.id, &assistant_message_id, &value);
      }
    }
...
fn apply_event_to_blocks(
  ...
  match event {
    ...
    EngineEvent::TurnCompleted { token_usage } => {
      ...
      progress.token_usage = token_usage.as_ref().map(|usage| (usage.input, usage.output));
    }
